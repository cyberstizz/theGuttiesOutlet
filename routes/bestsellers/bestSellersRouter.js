...
const bestSellerssRouter = express.Router({mergeParams: true});
...
// enabling cors
bestSellerssRouter.use(cors());
 
// allowing access to the requst.body
bestSellerssRouter.use(express.json());
...
console.log(`for this particular session this is visit numbeer ${req.session.visits}`)
 ...
 // everything is wrapped into a try catch block
    ...
    const fullListCall = await pool.query('select * from pictestertwo order by price desc limit 3')
  ...
  module.exports = bestSellerssRouter;
